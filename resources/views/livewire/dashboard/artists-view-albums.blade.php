<div class="row row--grid">
    ...
    <div class="col-12">
      <ul class="breadcrumb">
        ...
        <li class="breadcrumb__item"><a href="#!">Albums</a></li>
      </ul>
    </div>
    <!-- end breadcrumb -->
      ...
      <div class="release">
        <div class="release__content">
          <div class="release__cover">
            @if (is_null($songs[0]->album->album_cover))
                <img src="{{ asset('storage/app/public/'.$songs[0]->album->artist->photo) }}" alt="gggg">
                @else
                <img src="{{ asset('storage/app/public/'.$songs[0]->album->album_cover) }}" alt="tt">
            @endif
         
          </div>
          <div class="release__stat">
            <span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21.65,2.24a1,1,0,0,0-.8-.23l-13,2A1,1,0,0,0,7,5V15.35A3.45,3.45,0,0,0,5.5,15,3.5,3.5,0,1,0,9,18.5V10.86L20,9.17v4.18A3.45,3.45,0,0,0,18.5,13,3.5,3.5,0,1,0,22,16.5V3A1,1,0,0,0,21.65,2.24ZM5.5,20A1.5,1.5,0,1,1,7,18.5,1.5,1.5,0,0,1,5.5,20Zm13-2A1.5,1.5,0,1,1,20,16.5,1.5,1.5,0,0,1,18.5,18ZM20,7.14,9,8.83v-3L20,4.17Z"/></svg> {{ $songs->count() }} tracks</span>
            <span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21,14a1,1,0,0,0-1,1v4a1,1,0,0,1-1,1H5a1,1,0,0,1-1-1V15a1,1,0,0,0-2,0v4a3,3,0,0,0,3,3H19a3,3,0,0,0,3-3V15A1,1,0,0,0,21,14Zm-9.71,1.71a1,1,0,0,0,.33.21.94.94,0,0,0,.76,0,1,1,0,0,0,.33-.21l4-4a1,1,0,0,0-1.42-1.42L13,12.59V3a1,1,0,0,0-2,0v9.59l-2.29-2.3a1,1,0,1,0-1.42,1.42Z"></path></svg>{{ $songs[0]->downloads->count() }}</span>
          </div>
    
          <a href="#modal-buy" class="release__buy open-modal " style="cursor: no-drop">Download This Album</a>
        </div>
          ...
          <ul class="main__list main__list--playlist main__list--dashbox">
            ...
            @endphp
              ...
              <li class="single-item">
                <a data-playlist data-title="{{ $i }}. {{ $song->track }}" data-artist="{{ $song->album->artist->name }}" 
                  @if (is_null($song->album->album_cover))
                  data-img="{{ asset('storage/app/public/'.$song->album->artist->photo)}}"
                  @else
                  data-img="{{ asset('storage/app/public/'.$song->album->album_cover) }}" 
                  @endif
                  
                href="{{ asset('storage/app/public/'.$song->file_path) }}" class="single-item__cover">
                @if (is_null($song->album->album_cover))
                <img src="{{ asset('storage/app/public/'.$song->album->artist->photo) }}" alt="gggg">
                @else
                <img src="{{ asset('storage/app/public/'.$song->album->album_cover) }}" alt="tt">
                @endif
                ...
                <div class="single-item__title">
                  ...
                  <span><a href="#!">{{ $song->album->artist->name }}</a></span>
                </div>
                <div class="single-item__title">
                <span class="single-item__time">Uploaded by: {{ $song->uploaded_by }}</span>
                </div>
                <div class="btn-group">
                  <button wire:click="delete('{{ $song->id }}')" type="button" class="btn btn-sm btn-danger">
                    Delete
                  </button>
             ...
             <div class="col-sm-4 col-lg-3">
              <div class="album album--sidebar">
                <div class="album__cover">
                  ...
                  <a href="/dash/view-albums?album={{ $other_album->slug }}"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M18.54,9,8.88,3.46a3.42,3.42,0,0,0-5.13,3V17.58A3.42,3.42,0,0,0,7.17,21a3.43,3.43,0,0,0,1.71-.46L18.54,15a3.42,3.42,0,0,0,0-5.92Zm-1,4.19L7.88,18.81a1.44,1.44,0,0,1-1.42,0,1.42,1.42,0,0,1-.71-1.23V6.42a1.42,1.42,0,0,1,.71-1.23A1.51,1.51,0,0,1,7.17,5a1.54,1.54,0,0,1,.71.19l9.66,5.58a1.42,1.42,0,0,1,0,2.46Z"/></svg></a>
                ...
                <div class="album__title">
                  <h3><a href="/dash/view-albums?album={{ $other_album->slug }}">{{ $other_album->title }}</a></h3>
                  <span><a href="/dash/view-albums?album={{ $other_album->slug }}">{{ $other_album->artist->name }}</a></span>
                </div>
              </div>
            </div>
             @empty
