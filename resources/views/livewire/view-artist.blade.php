    ...
    <div class="row row--grid">    
      ...
      <div class="col-12 ">
        <div class="main__title main__title--page">
          <h1>{{ Str::ucfirst($albums->artist->name) }} - {{ Str::ucfirst($albums->title) }}</h1>
        ...
        <div class="release">
          <div class="release__content">
            <div class="release__cover">
              <img src="{{ asset('storage/app/public/'.$albums->artist->photo) }}" alt="">
            </div>
            
            <div class="release__stat">
              <span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21.65,2.24a1,1,0,0,0-.8-.23l-13,2A1,1,0,0,0,7,5V15.35A3.45,3.45,0,0,0,5.5,15,3.5,3.5,0,1,0,9,18.5V10.86L20,9.17v4.18A3.45,3.45,0,0,0,18.5,13,3.5,3.5,0,1,0,22,16.5V3A1,1,0,0,0,21.65,2.24ZM5.5,20A1.5,1.5,0,1,1,7,18.5,1.5,1.5,0,0,1,5.5,20Zm13-2A1.5,1.5,0,1,1,20,16.5,1.5,1.5,0,0,1,18.5,18ZM20,7.14,9,8.83v-3L20,4.17Z"/></svg> {{ $albums->songs->count() }} tracks</span>
              <div  wire:loading wire:target='downloadEachSong' style="color: green"><span>Downloading... <i class="bi bi-arrow-clockwise spin"></i></span></div>
              <span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21,14a1,1,0,0,0-1,1v4a1,1,0,0,1-1,1H5a1,1,0,0,1-1-1V15a1,1,0,0,0-2,0v4a3,3,0,0,0,3,3H19a3,3,0,0,0,3-3V15A1,1,0,0,0,21,14Zm-9.71,1.71a1,1,0,0,0,.33.21.94.94,0,0,0,.76,0,1,1,0,0,0,.33-.21l4-4a1,1,0,0,0-1.42-1.42L13,12.59V3a1,1,0,0,0-2,0v9.59l-2.29-2.3a1,1,0,1,0-1.42,1.42Z"></path></svg> {{ $albums->songs[0]->downloads->count() }}</span>
              </div>
            <div wire:loading.remove wire:target="downloadAlbum"style="display: flex">
              <a wire:click="downloadAlbum('{{ $albums->slug }}')" href="#!" class="release__buy">Download album</a>
            </div>
            <div wire:loading.flex wire:target="downloadAlbum">
              <a href="#!" class="release__buy" style="background-color: green">Downloading Please wait...</a>
            </div>
          </div>
...
@endphp
              <li class="single-item">
                <a  data-playlist data-title="{{ $i }}. {{ $song->track }}" data-artist="{{ Str::ucfirst($albums->artist->name) }}" 
                    data-img="{{ asset('storage/app/public/'.$albums->artist->photo) }}"
                     href="{{ asset('storage/app/public/'.$song->file_path) }}" class="single-item__cover">
                  <img src="{{ asset('storage/app/public/'.$albums->artist->photo) }}" alt="">
                ...
                <div class="single-item__title">
                  <h4><a href="#">{{ $i }}. {{ $song->track }}</a></h4>
                  <span><a href="#!">Uploaded By: {{ $song->uploaded_by }}</a></span>
                </div>
               
                @auth
                <a wire:click="addToPlaylist('{{ $song->slug }}')" href="#" class="single-item__add" data-toggle="tooltip" data-placement="left" title="Add To Playlist">
                ...
                @endauth
                   <div wire:loading.remove>
                      <a wire:click="downloadEachSong('{{ $song->slug }}')" data-toggle="tooltip" data-placement="top" title="Download" href="#!" class="single-item__export">
                ...
                <span class="single-item__time">{{ round(filesize('storage/app/public/'.$song->file_path) / 1024 / 1024)}} Mb</span>
                <span class="single-item__time text-danger">{{ $estimated_song_duration }}</span>
              </li> 
      ...
      <div class="col-12 col-lg-12">
        <div class="article">
          ...
          <div class="row row--sidebar">
            ...
            <!-- end title -->
  @forelse ($other_albums as $other_album)
  <div class="col-4 col-sm-4 col-lg-3">
    <div class="album album--sidebar">
      <div class="album__cover">
        @if (!is_null($other_album->album_cover))
        <img src="{{ asset('storage/app/public/'.$other_album->album_cover) }}" alt="">
        @else
        <img src="{{ asset('storage/app/public/'.$other_album->artist->photo) }}" alt="">
        @endif
        
        <a href="view-album?album={{ $other_album->artist->slug }}"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M18.54,9,8.88,3.46a3.42,3.42,0,0,0-5.13,3V17.58A3.42,3.42,0,0,0,7.17,21a3.43,3.43,0,0,0,1.71-.46L18.54,15a3.42,3.42,0,0,0,0-5.92Zm-1,4.19L7.88,18.81a1.44,1.44,0,0,1-1.42,0,1.42,1.42,0,0,1-.71-1.23V6.42a1.42,1.42,0,0,1,.71-1.23A1.51,1.51,0,0,1,7.17,5a1.54,1.54,0,0,1,.71.19l9.66,5.58a1.42,1.42,0,0,1,0,2.46Z"/></svg></a>
        <span class="album__stat">
          <span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M3.71,16.29a1,1,0,0,0-.33-.21,1,1,0,0,0-.76,0,1,1,0,0,0-.33.21,1,1,0,0,0-.21.33,1,1,0,0,0,.21,1.09,1.15,1.15,0,0,0,.33.21.94.94,0,0,0,.76,0,1.15,1.15,0,0,0,.33-.21,1,1,0,0,0,.21-1.09A1,1,0,0,0,3.71,16.29ZM7,8H21a1,1,0,0,0,0-2H7A1,1,0,0,0,7,8ZM3.71,11.29a1,1,0,0,0-1.09-.21,1.15,1.15,0,0,0-.33.21,1,1,0,0,0-.21.33.94.94,0,0,0,0,.76,1.15,1.15,0,0,0,.21.33,1.15,1.15,0,0,0,.33.21.94.94,0,0,0,.76,0,1.15,1.15,0,0,0,.33-.21,1.15,1.15,0,0,0,.21-.33.94.94,0,0,0,0-.76A1,1,0,0,0,3.71,11.29ZM21,11H7a1,1,0,0,0,0,2H21a1,1,0,0,0,0-2ZM3.71,6.29a1,1,0,0,0-.33-.21,1,1,0,0,0-1.09.21,1.15,1.15,0,0,0-.21.33.94.94,0,0,0,0,.76,1.15,1.15,0,0,0,.21.33,1.15,1.15,0,0,0,.33.21,1,1,0,0,0,1.09-.21,1.15,1.15,0,0,0,.21-.33.94.94,0,0,0,0-.76A1.15,1.15,0,0,0,3.71,6.29ZM21,16H7a1,1,0,0,0,0,2H21a1,1,0,0,0,0-2Z"/></svg> {{ $other_album->songs->count() }}</span>
          {{-- <span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M20,13.18V11A8,8,0,0,0,4,11v2.18A3,3,0,0,0,2,16v2a3,3,0,0,0,3,3H8a1,1,0,0,0,1-1V14a1,1,0,0,0-1-1H6V11a6,6,0,0,1,12,0v2H16a1,1,0,0,0-1,1v6a1,1,0,0,0,1,1h3a3,3,0,0,0,3-3V16A3,3,0,0,0,20,13.18ZM7,15v4H5a1,1,0,0,1-1-1V16a1,1,0,0,1,1-1Zm13,3a1,1,0,0,1-1,1H17V15h2a1,1,0,0,1,1,1Z"/></svg> {{ $other_album->songs-> }}</span> --}}
        </span>
      </div>
      <div class="album__title">
        <h3><a href="view-album?album={{ $other_album->slug }}">{{ $other_album->title }}</a></h3>
        <span><a href="view-album?album={{ $other_album->slug }}">{{ $other_album->artist->name }}</a> </span>
      </div>
    </div>
  </div>
  @empty
            ...
            <ul class="comments__list">
              ...
              <li class="comments__item">
                <div class="comments__autor">
                  @if (is_null($comment->user->picture))
                  <img class="comments__avatar" src="https://eu.ui-avatars.com/api/?background=random&amp;name={{ Str::ucfirst($comment->user->first_name) }} 
                  {{ Str::ucfirst($comment->user->last_name) }}" alt="">
                  @else
                  <img class="comments__avatar" src="{{ asset('storage/app/public/'.$comment->user->picture) }}" alt="">
                  @endif
                ...
                <div class="comments__actions">
                  <div class="comments__rate">
                    <button type="button"><i class="bi bi-hand-thumbs-up text-success"></i> 12</button>

                    <button type="button">7 <i class="bi bi-hand-thumbs-down text-danger"></i></button>
                  </div>
...
@auth
<form wire:submit.prevent='storeComment("{{ Crypt::encrypt($albums->artist->id) }}")' class="comments__form">
  @csrf
  <div class="sign__group">
    <textarea wire:model='comment' id="text" class="sign__textarea" placeholder="Add comment"></textarea>
    @error('comment')
      <p class="text-danger">{{ $message }}</p>
    @enderror
  </div>
  <div wire:loading.inline wire:target="storeComment">
    <button type="submit" class="sign__btn">Loading...</button>
</div>
  <button wire:loading.remove wire:target="storeComment" type="submit" class="sign__btn">Send</button>
</form>
@endauth
